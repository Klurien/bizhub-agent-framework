...
program
  .command("login")
  .description("Log into your BizHub account")
  .action(async () => {
    const cfg = loadConfig();
    const apiUrl = cfg.apiUrl || "http://localhost:3001";

    const i = rl();
    const email = await i.question(chalk.dim("  Email: "));
    const password = await i.question(chalk.dim("  Password: "));
    i.close();
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.log(chalk.red("✖ Login failed:"), data.message || `HTTP ${res.status}`);
        return;
      }
      ...
      if (!match) {
        console.log(chalk.red("✖ Login failed: no session token returned"));
        return;
      }
      ...
      console.log(chalk.green(`✔ Logged in as ${chalk.bold(data.user?.name || email)}`));
      console.log(chalk.dim(`  Role: ${data.user?.role} · Dashboard: ${data.user?.role === "ADMIN" ? "/panel" : "/dashboard"}`));
    } catch (e) {
      console.log(chalk.red("✖ Connection error:"), (e as Error).message);
    }
  });
...
program
  ...
  .action(() => {
    const cfg = loadConfig();
    if (cfg.authCookie) {
      console.log(chalk.green("✔ Logged in as"), chalk.bold(cfg.userName || cfg.userEmail));
      console.log(chalk.dim(`  Email: ${cfg.userEmail}`));
      console.log(chalk.dim(`  Role: ${cfg.userRole}`));
      console.log(chalk.dim(`  User ID: ${cfg.userId?.slice(0, 12)}...`));
    } else {
      console.log(chalk.yellow("  Not logged in. Run"), chalk.cyan("bizhub login"), chalk.yellow("first."));
    }
  });

program
  .command("logout")
  .description("Log out of your BizHub account")
  .action(() => {
    saveConfig({ authCookie: "", userEmail: "", userName: "", userRole: "", userId: "" });
    console.log(chalk.green("✔ Logged out"));
  });

// ─── Marketplace Commands (from agent-kit) ──────────────────

import { BizHubAgent, getConfig, api } from "@biz-hub/agent-kit";
import Table from "cli-table3";

const agent = new BizHubAgent({ name: "bizhub-cli", version: pkg.version });
agent.loadDefaultTools();

async function ensureAuth(): Promise<string | false> {
  const cfg = getConfig();
  if (cfg.authCookie) {
    api.setAuth(cfg.authCookie);
    return cfg.authCookie;
  }
  console.log(chalk.yellow("  Not logged in. Run"), chalk.cyan("bizhub login"), chalk.yellow("first."));
  return false;
}
...
products.command("list").description("List products").option("-c, --category <slug>", "Filter by category").option("--sort <field>", "Sort order").option("-l, --limit <n>", "Max results", "50").option("--json", "JSON output").action(async (o) => {
  if (!(await ensureAuth())) return console.log(chalk.red("✖ Not authenticated. Run `bizhub dashboard` first."));
...
products.command("get <slug>").description("Get product details").option("--json", "JSON output").action(async (slug, o) => {
  if (!(await ensureAuth())) return console.log(chalk.red("✖ Not authenticated."));
...
program
  ...
  .action(async () => {
    ...
    const token = cfg.authCookie || "";
    if (!token) {
      console.log(chalk.yellow("  Not logged in. Run"), chalk.cyan("bizhub login"), chalk.yellow("first."));
      return;
    }

    const role = cfg.userRole || "USER";
    const path = role === "ADMIN" ? "/panel" : "/dashboard";
    const dashboardUrl = `${url}${path}?token=${token}`;

    console.log(chalk.cyan("  → Opening"), chalk.white(dashboardUrl));
    try {
      const cmd = platform() === "win32" ? "start" : platform() === "darwin" ? "open" : "xdg-open";
      execSync(`${cmd} "${dashboardUrl}"`, { stdio: "ignore" });
    } catch {
      console.log(chalk.yellow("  ⚠ Could not open browser. Visit:"), chalk.cyan(dashboardUrl));
    }
  });
